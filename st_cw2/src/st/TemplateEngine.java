        ...
        for (Integer i=0; i<sortedTemplates.size(); i++){
            ...
            for(Integer j=0; j<sortedEntries.size(); j++){
                ...
                if (isAMatch(currentTemplate, currentEntry, matchingMode)){
                	/**
                	 * We need to implement those two functions
                	 * isYearTemplate checks if the current entry is a "year" template
                	 */
                	if(isYearTemplate(currentEntry)) {
                		/**
                		 * replaceYearEntry, updates the value of the entry with the coresponding 
                		 * value of each cate. No need to return anything, just update the value
                		 */
                    	replaceYearEntry(sortedEntries,currentEntry,matchingMode);
                    }
