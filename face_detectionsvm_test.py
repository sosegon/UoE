...
from common import try_predictionsvm, load_clf
...
def evaluate_classifier(clf_name, stride, prob_tresh):
    ...
    val_path = "./test_files/data/face_detection/te_raw_images/*/*"
    val_paths = glob.glob(val_path)

    with open("{:s}_test.log".format(clf_name), "w+") as log_file:
...
#########################################################################################
desc = """ Test a SVM classifier for face detection.
