...
An overview of the general aspects of the design has been described previously (\ref{sec:design}). However, specific details about use case, features, user interface components, and user interactions are still required. The objective of such specifications is to clearly establish the shape of the final application. The output of this stage will be a specifications document describing the aforementioned details.

\subsection{Implementation of the design}
This stage corresponds to implementing the application based on the output from the previous stage. This phase is divide into iterative phases. The end of each phase will deliver an updated version of the application. Each new version will contain additional features and will fix bugs from the previous ones.
...
Throughout the development and evaluation processes, there might a series of potential risks that can delay or compromise the flow of the project. To diminish the impact of such potential risks it is necessary to identify and categorize them. Moreover, an strategy to cope with them is needed.
